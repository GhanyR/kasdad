...
import { SwipeNavigationContext, useSwipeNavigationProvider } from "@/lib/SwipeNavigationContext";
...
function useSwipeNavigation(
  ...
  const [swipeIndicator, setSwipeIndicator] = useState<{ dir: "left" | "right"; label: string } | null>(null);
  ...
  const handleTouchEnd = useCallback((e: TouchEvent) => {
    ...
    if (dx < 0) {
      // Swipe left = go NEXT (next tab, or next material)
      const handledByTab = handleSwipeNext();
      if (!handledByTab && nextSlug) {
        setSwipeIndicator({ dir: "left", label: "Next material →" });
        setTimeout(() => {
      ...
      // Swipe right = go PREV (prev tab, or prev material)
      const handledByTab = handleSwipePrev();
      if (!handledByTab && prevSlug) {
        setSwipeIndicator({ dir: "right", label: "← Prev material" });
        setTimeout(() => {
  ...
  }, [nextSlug, prevSlug, router, handleSwipeNext, handleSwipePrev]);
...
export default function MateriPage({
  ...
  const { register, handleSwipeNext, handleSwipePrev } = useSwipeNavigationProvider();

  const { swipeIndicator } = useSwipeNavigation(
    prev?.slug ?? null,
    next?.slug ?? null,
    handleSwipeNext,
    handleSwipePrev,
  ...
  return (
    <SwipeNavigationContext.Provider value={{ register, handleSwipeNext, handleSwipePrev }}>
      <div style={{ position: "relative" }}>
        {/* Swipe indicator toast */}
        {swipeIndicator && (
          <div style={{
        ...
        <div style={{
          ...
          {prev ? (
            <Link href={`/materi/${prev.slug}`} style={{
              ...
              {!isMobile && <span style={{ overflow: "hidden", textOverflow: "ellipsis" }}>{prev.title}</span>}
            </Link>
          ) : <div />}
          <Link href="/" style={{
          ...
          {next ? (
            <Link href={`/materi/${next.slug}`} style={{
              ...
              {!isMobile && <span style={{ overflow: "hidden", textOverflow: "ellipsis" }}>{next.title}</span>}
...
function SwipeHint({ isMobile }: { isMobile: boolean }) {
  const [show, setShow] = useState(false);

  useEffect(() => {
    if (!isMobile) return;
    try {
      const seen = localStorage.getItem("kasdad-swipe-hint-v2");
      if (!seen) {
        setShow(true);
        localStorage.setItem("kasdad-swipe-hint-v2", "1");
  ...
  return (
    <div style={{
      ...
      <div style={{ fontSize: 14, fontWeight: 600, marginBottom: 6 }}>Swipe untuk navigasi</div>
      <div style={{ fontSize: 12, opacity: 0.8 }}>← → antar sub-topik, lalu ke materi berikutnya</div>
    </div>
  );
}
